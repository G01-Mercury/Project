...
test('parseJwt function should decode JWT token', () => {
  ...
  expect(decodedToken.name).toBe('Prova Prova');
  expect(decodedToken.email).toBe('provamercury8@gmail.com'); 
  expect(decodedToken.given_name).toBe('Prova'); 
  expect(decodedToken.family_name).toBe('Prova'); 
  expect(decodedToken.picture).toBe('https://lh3.googleusercontent.com/a/ACg8ocLU7VahiezdEGweHNmHLrkbklJrA4gItgtEnHvxT_4Q=s96-c'); 
});
...
test('onsignin function should process the response', () => {
  ...
  expect(paramValue).toBe('118368447079240633788,Prova Prova,provamercury8@gmail.com,Prova,Prova,https://lh3.googleusercontent.com/a/ACg8ocLU7VahiezdEGweHNmHLrkbklJrA4gItgtEnHvxT_4Q=s96-c');
});
*/